...
export const getFavoriteCities = () => {
	...
	try {
		...
		return data ? JSON.parse(data).filter((city) => typeof city === 'string') : [];
	} catch (error) {
...
export const updateFavoriteCity = (oldCity, newCity) => {
	...
	try {
		const oldCityName = typeof oldCity === 'string' ? oldCity.trim().toLowerCase() : '';
		...
		const favorites = getFavoriteCities();
		const index = favorites.findIndex((fav) => fav.toLowerCase() === oldCityName);
		if (index === -1) {
...
export const removeFavoriteCity = (city) => {
	...
	try {
		...
		const favorites = getFavoriteCities();
		const updatedFavorites = favorites.filter((fav) => fav.toLowerCase() !== cityName);
